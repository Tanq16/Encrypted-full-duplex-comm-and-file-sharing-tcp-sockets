...
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    if(filewrite == True):
        salsaenc = Salsa20.new(salsakey, nonce=(salsanonce + bytes(str("%07d"%x), "utf_8")))
        salsadec = Salsa20.new(salsakey, nonce=(salsanonce + bytes(str("%07d"%x), "utf_8")))
        mlen = int(str(salsadec.decrypt(s.recv(20)), 'utf_8'))
        response = str(salsadec.decrypt(s.recv(mlen)), 'utf_8')
        ...
        salsaenc = Salsa20.new(salsakey, nonce=(salsanonce + bytes(str("%07d"%x), "utf_8")))
        salsadec = Salsa20.new(salsakey, nonce=(salsanonce + bytes(str("%07d"%x), "utf_8")))
        s.send(salsaenc.encrypt(bytes((messagelen + message), "utf_8")))
        s.close()
...
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    if(filewrite == True):
        salsaenc = Salsa20.new(salsakey, nonce=(salsanonce + bytes(str("%07d"%x), "utf_8")))
        salsadec = Salsa20.new(salsakey, nonce=(salsanonce + bytes(str("%07d"%x), "utf_8")))
        mlen = int(str(salsadec.decrypt(sock.recv(20)), 'utf_8'))
        response = str(salsadec.decrypt(sock.recv(mlen)), 'utf_8')
        with open (filename, 'w') as myfile:
            myfile.write(response)
        sock.close()
        ss.close()
        ...
        salsaenc = Salsa20.new(salsakey, nonce=(salsanonce + bytes(str("%07d"%x), "utf_8")))
        salsadec = Salsa20.new(salsakey, nonce=(salsanonce + bytes(str("%07d"%x), "utf_8")))
        sock.send(salsaenc.encrypt(bytes((messagelen + message), "utf_8")))
        sock.close()
        ss.close()
